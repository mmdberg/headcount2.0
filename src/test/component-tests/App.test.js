...
describe('App', () => {
  ...
  it('should update stats in state with all districts that match user input', () => {
    wrapper.instance().filterStats('COL');
    expect(wrapper.state('stats').length).toEqual(2);
  })  
  
  it('should remove the card from state if selected', () => {
    const mockState = {
      stats: [],
      selectedCards: [{
        location: "COLORADO",
  ...
  it('should add the card to state if not selected', () => {
    const mockData = {
      location: "COLORADO",
      className: "card"
    };
  ...
  it('should replace the second card if two cards are selected', () => {
    const mockState = {
      stats: [],
      selectedCards: [{
        location: "COLORADO",
